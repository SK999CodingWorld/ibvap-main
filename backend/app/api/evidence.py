...
async def get_evidence_snapshot(filename: str):
    ...
    Secured against Path Traversal (CWE-22) vulnerabilities.
    """
    # Sanitize filename to prevent directory traversal
    clean_filename = os.path.basename(filename)
    filepath = os.path.abspath(os.path.join(evidence_vault.storage_dir, clean_filename))
    storage_abs = os.path.abspath(evidence_vault.storage_dir)

    # Verify canonical path remains within storage_dir
    if not filepath.startswith(storage_abs):
        raise HTTPException(status_code=400, detail="Invalid snapshot path parameter")

    if not os.path.exists(filepath):
        # Fallback to demo snapshot if exact file not found
        if os.path.exists(evidence_vault.storage_dir):
