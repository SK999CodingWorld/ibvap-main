      ...
      ws.onmessage = (event) => {
        try {
          ...
          const newAlert: AlertItem = {
            ...
            riskScore: data.severity === 'CRITICAL' ? 88 : 65,
            object: data.object_type?.toUpperCase() || 'TARGET',
            trackingId: data.track_id || 'N/A'
          };
          if (isMounted) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(alertsData) && alertsData.length > 0 && isMounted) {
            const formatted: AlertItem[] = alertsData.slice(0, 20).map((a, i) => ({
              id: `ALT-${8000 + i}`,
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 text-slate-200 overflow-y-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {kpis.map((kpi, i) => (
          <div key={i} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between hover:border-slate-700 transition-colors shadow">
            <div className="flex justify-between items-start">
              <span className="text-[11px] text-slate-400 font-semibold uppercase">{kpi.label}</span>
            ...
            <div className="mt-3 flex justify-between items-baseline">
              ...
              <span className="text-[10px] text-slate-500 font-mono">{kpi.change}</span>
            </div>
          </div>
        ))}
      </div>
